#!/usr/bin/python
# Modified from https://github.com/brendangregg/FlameGraph/blob/master/stackcollapse-chrome-tracing.py
# to accept the format outputed by the neovim plugin stevearc/profile.nvim as that plugin doesn't writes a
# json array of events instead of having them on the traceEvents field
#
...
# ]}
#
# Example input without traceEvents:
# [
#     {"pid":1,"tid":2,"ts":0,"ph":"X","name":"Foo","dur":50},
#     {"pid":1,"tid":2,"ts":10,"ph":"X","name":"Bar","dur":30}
# ]
...
def get_trace_events(trace_file, events_dict):
    ...
    for entry in events:
        if entry["ph"] == "X":
            ...
            events_dict[cantor_val].append(
                Event(entry["name"], float(entry["ts"]), float(entry["dur"]))
            )
