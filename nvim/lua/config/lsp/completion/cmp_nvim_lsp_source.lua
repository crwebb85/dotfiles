--From https://github.com/hrsh7th/cmp-nvim-lsp/blob/a8912b88ce488f411177fc8aed358b04dc246d7b/lua/cmp_nvim_lsp/source.lua
local source = {}
...
source.is_available = function(self)
    ...
    if
        vim.tbl_isempty(
            vim.lsp.get_clients({ bufnr = bufnr, id = self.client.id })
        )
    then
        return false
    end
    ...
    if
        not self:_get(self.client.server_capabilities, { 'completionProvider' })
    then
        return false
    end
    return true
...
source.get_position_encoding_kind = function(self)
    return self:_get(self.client.server_capabilities, { 'positionEncoding' })
        or self.client.offset_encoding
        or 'utf-16'
end
...
source.complete = function(self, params, callback)
    ...
    self:_request(
        ...
        lsp_params,
        function(_, response) callback(response) end
    )
end
...
source.resolve = function(self, completion_item, callback)
    -- client is stopped.
    if self.client.is_stopped() then return callback() end
    ...
    self:_request(
        ...
        completion_item,
        function(_, response) callback(response or completion_item) end
    )
end
...
source.execute = function(self, completion_item, callback)
    ...
    if not completion_item.command then return callback() end

    self:_request(
        'workspace/executeCommand',
        completion_item.command,
        function(_, _) callback() end
    )
end
...
source._request = function(self, method, params, callback)
    ...
    local _, request_id
    _, request_id = self.client.request(
        method,
        params,
