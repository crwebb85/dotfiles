...
function M.format_on_save(opts)
    ...
    local filetype_setup = function(event)
        ...
        if type(files) == 'string' then files = { list[client.name] } end

        if
            files == nil
            or vim.tbl_contains(files, vim.bo.filetype) == false
        then
            return
        end
...
function M.async_autoformat(client, bufnr, opts)
    if type(client) ~= 'table' or client.id == nil then return end
...
function M.format_mapping(key, opts)
    if opts == nil or key == nil then return end
    ...
    if vim.tbl_isempty(list) then return end
...
function M.check(server)
    ...
    local client =
        vim.lsp.get_active_clients({ bufnr = buffer, name = server })[1]
    ...
    if client.supports_method('textDocument/formatting') == false then
        local msg =
            '[lsp-zero] %s does not support textDocument/formatting method'
        vim.notify(msg:format(server), vim.log.levels.WARN)
        return
    end
...
function s.request_format(client_id, buffer, format_opts, timeout)
    ...
    if vim.b.lsp_zero_changedtick == vim.b.changedtick then return end

    if timeout <= 0 then timeout = timeout_ms end
    ...
    local handler = function(err, result, ctx)
        if timer == nil or timer:get_due_in() == 0 then return end
...
function s.format_handler(err, result, ctx)
    ...
    if vim.fn.bufexists(buffer) == 0 then return end
    ...
    if current_changedtick ~= buf_changedtick then
        local msg =
            '[lsp-zero] Format canceled. Buffer was modified after request.'
        vim.notify(msg, vim.log.levels.WARN)
        buf_set(buffer, fmt_var, 0)
        return
    end
...
function s.format_cleanup(buffer, client_name)
    ...
    local msg =
        '[lsp-zero] Format request timeout. %s is taking too long to respond.'
