...
function M.setup(user_opts)
    if type(user_opts) ~= 'table' then user_opts = {} end
    ...
    if opts.preselect == false then vim.opt.completeopt:append('noselect') end

    if opts.verbose == false then vim.opt.shortmess:append('c') end
    ...
    if set_autocomplete then vim.opt.completeopt:append('noinsert') end

    if opts.use_fallback then
        if set_autocomplete then M.autocomplete_fallback() end

        if set_tabcomplete then M.tab_complete_fallback() end

        if set_toggle then M.toggle_menu_fallback(set_toggle) end
    end
    ...
    autocmd('LspAttach', {
        ...
        callback = function(event)
            if set_autocomplete then M.autocomplete(event.buf) end

            if set_tabcomplete then M.tab_complete(event.buf) end

            if set_toggle then M.toggle_menu(set_toggle, event.buf) end

            if map_backspace then s.backspace(event.buf) end
        end,
    })
end
...
function s.try_complete()
    if pumvisible() > 0 or s.is_macro() then return end
...
function s.try_complete_fallback()
    if pumvisible() > 0 or s.is_macro() or s.is_prompt() then return end
...
function s.backspace(buffer)
    local rhs = function()
        if pumvisible() == 1 then return '<bs><c-x><c-o>' end
...
function s.keymap(lhs, action)
    if lhs == nil or string.lower(lhs) == string.lower(action) then return end

    local rhs = function()
        if pumvisible() == 1 then return action end
...
function s.tab_expr()
    if pumvisible() == 1 then return key.next_item end

    if s.has_words_before() then return key.omni end
...
function s.complete_words()
    ...
    if s.has_words_before() then return key.buffer end

    return key.tab
end

function s.prev_item()
    if pumvisible() == 1 then return key.prev_item end

    return key.tab
end

function s.toggle_expr()
    if pumvisible() == 1 then return key.abort end
