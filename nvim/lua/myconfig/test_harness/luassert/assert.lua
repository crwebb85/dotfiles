    ...
    __call = function(self, ...)
        ...
        if assertion then
            ...
            if (not val) == self.mod then
                ...
                if not self.mod then message = assertion.negative_message end
                local err = geterror(
                    message,
                    rawget(self, 'failure_message'),
                    arguments
                )
                error(err or 'assertion failed!', util.errorlevel())
            end

            if retargs then return unpack(retargs) end
            return ...
        else
            local arguments = util.make_arglist(...)
            self.tokens = {}
            ...
            for _, key in ipairs(keys) do
                if namespace.modifier[key] then
                    namespace.modifier[key].callback(
                        self,
                        arguments,
                        util.errorlevel()
                    )
...
obj = {
    ...
    register = function(
        ...
        if not namespace[nspace] then namespace[nspace] = {} end
