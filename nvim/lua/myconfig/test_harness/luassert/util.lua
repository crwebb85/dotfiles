...
function util.deepcompare(t1, t2, ignore_mt, cycles, thresh1, thresh2)
    ...
    if cycles[1][t1] > thresh1 and cycles[2][t2] > thresh2 then return true end
    ...
    for k1, v1 in next, t1 do
        local v2 = t2[k1]
        if v2 == nil then return false, { k1 } end

        local same, crumbs =
            util.deepcompare(v1, v2, nil, cycles, thresh1, thresh2)
        if not same then
            crumbs = crumbs or {}
...
function util.deepcopy(t, deepmt, cache)
    local spy = require('myconfig.test_harness.luassert.spy')
    if type(t) ~= 'table' then return t end
    local copy = {}
    ...
    if deepmt then
        debug.setmetatable(
            copy,
            util.deepcopy(debug.getmetatable(t), false, cache)
        )
    else
...
function util.copyargs(args)
    ...
    local match = require('myconfig.test_harness.luassert.match')
    local spy = require('myconfig.test_harness.luassert.spy')
    for k, v in pairs(args) do
        copy[k] = (
            (match.is_matcher(v) or spy.is_spy(v)) and v or util.deepcopy(v)
        )
    end
...
local function matcharg(args, argrefs, specs)
    ...
    for idx, argval in pairs(args) do
        ...
        if match.is_matcher(spec) then
            if match.is_ref_matcher(spec) then argval = argrefs[idx] end
            if not spec(argval) then return false end
...
function util.matchargs(invocations_list, specs)
    ...
    for ii = #invocations_list, 1, -1 do
        ...
        if matcharg(val.vals, val.refs, specs) then return val end
    end
    return nil
end
...
function util.matchoncalls(oncalls, args)
    for _, callspecs in ipairs(oncalls) do
        ...
        -- either.
        if matcharg(args, args, callspecs.vals) then return callspecs end
    end
    return nil
end
...
function util.extract_keys(nspace, tokens)
    local namespace = require('myconfig.test_harness.luassert.namespaces')
