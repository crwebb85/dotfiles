...
local astate = require('myconfig.test_harness.luassert.state')
local util = require('myconfig.test_harness.luassert.util')
local s = require('myconfig.test_harness.say')
...
local function set_failure_message(state, message)
    if message ~= nil then state.failure_message = message end
end
...
local function unique(state, arguments, level)
    ...
    then
        ...
        if type(arguments[3]) == 'boolean' then deep = arguments[3] end
...
local function same(state, arguments, level)
    ...
    if type(arguments[1]) == 'table' and type(arguments[2]) == 'table' then
        local result, crumbs =
            util.deepcompare(arguments[1], arguments[2], true)
        -- switch arguments for proper output message
        util.tinsert(arguments, 1, util.tremove(arguments, 2))
...
local function has_error(state, arguments, level)
    ...
    if ok or err_expected == nil then return not ok, retargs end
...
local function error_matches(state, arguments, level)
    ...
    if err_actual == nil and pattern == nil then return true, {} end
