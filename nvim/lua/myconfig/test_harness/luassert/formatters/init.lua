...
do
    ...
    if not isatty then
        ...
        if isWindows and os.getenv('ANSICON') then isatty = true end
    end
...
local function fmt_string(arg)
    if type(arg) == 'string' then return string.format("(string) '%s'", arg) end
end

-- A version of tostring which formats numbers more precisely.
local function tostr(arg)
    if type(arg) ~= 'number' then return tostring(arg) end
...
local function fmt_nil(arg)
    if type(arg) == 'nil' then return '(nil)' end
end
...
local function fmt_table(arg, fmtargs)
    ...
    local function ft(t, l, with_crumbs)
        ...
        if l > tmax and tmax >= 0 then return '{ ... more }' end
...
local function fmt_arglist(arglist)
    if not util.is_arglist(arglist) then return end
