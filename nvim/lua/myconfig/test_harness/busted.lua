-- based on https://github.com/nvim-lua/plenary.nvim/blob/74b06c6c75e4eeb3108ec01852001636d85a932b/lua/plenary/busted.lua
--- changes primarily involved fixing lsp warnings and depreciations
--- also removed the global versions of these test functions
local function get_trace(_, level, msg)
    ...
    while
        ...
        or (
            info.source:sub(1, 1) == '@'
            and thisdir == vim.fn.fnamemodify(info.source, ':h')
        )
    do
        level = level + 1
...
local color_string = function(color, str)
    if not is_headless() then return str end
...
local indent = function(msg, spaces)
    if spaces == nil then spaces = 4 end
...
mod.assert = require('myconfig.test_harness.luassert')

--Note: this is called by a vim command not lua so it won't show up in lsp references
    ...
    if not loaded then
        ...
        coroutine.wrap(function()
            ...
            if #results.errs ~= 0 then
                ...
                if is_headless() then return vim.cmd('2cq') end
            elseif #results.fail > 0 then
                print('Tests Failed. Exit: 1')

                if is_headless() then return vim.cmd('1cq') end
            else
                if is_headless() then return vim.cmd('0cq') end
            end
        end)()
    end
end
