-- based on https://github.com/nvim-lua/plenary.nvim/blob/74b06c6c75e4eeb3108ec01852001636d85a932b/lua/plenary/job.lua
--- changes primarily involved fixing lsp warnings, depreciations, and updating type definitions

---@class TestHarnessJobOptions
...
---@field on_start? fun()
---@field on_stdout? fun(error: string, data: string, self?: TestHarnessJob)
---@field on_stderr? fun(error: string, data: string, self?: TestHarnessJob)
---@field on_exit? fun(self: TestHarnessJob, code: number, signal: number)
---@field maximum_results? number Stop processing results after this number
---@field writer? TestHarnessJob|table|string Job that writes to stdin of this job.
...
---@field maximum_results? number Stop processing results after this number
---@field writer? TestHarnessJob|table|string Job that writes to stdin of this job.
---@field private _user_on_exit? fun(self: TestHarnessJob, code: number, signal: number)
---@field private _additional_on_exit_callbacks? fun(self: TestHarnessJob, code: number, signal: number)[]
---@field private _raw_cwd string?
---@field private _prepare_pipes fun(self: TestHarnessJob)
---@field private _user_on_start? fun()
---@field private _user_on_stdout? fun(error: string, data: string, self?: TestHarnessJob)
---@field private _user_on_stderr? fun(error: string, data: string, self?: TestHarnessJob)
---@field private _user_on_exit? fun(self: TestHarnessJob, code: number, signal: number)
---@field stdout uv.uv_pipe_t?
---@field stdin uv.uv_pipe_t|{write: fun()}?
...
local start_shutdown_check = function(child, options, code, signal)
    vim.uv.check_start(child._shutdown_check, function()
        if not child:_pipes_are_closed(options) then return end

        -- Wait until all the pipes are closing.
        vim.uv.check_stop(child._shutdown_check)
...
local shutdown_factory = function(child, options)
    return function(code, signal)
        if vim.uv.is_closing(child._shutdown_check) then
...
---Create a new job
---@param o TestHarnessJobOptions
---@return TestHarnessJob
function Job:new(o)
    if not o then error(debug.traceback('Options are required for Job:new')) end
    ...
    local args = o.args
    if not args then
        if #o > 1 then args = { select(2, unpack(o)) } end
    end
    ...
    if o.env then
        if type(o.env) ~= 'table' then
            error('[test_harness.job] env has to be a table')
        end
    ...
    if o.detached then obj.detached = true end

    -- enable_handlers: Do you want to do ANYTHING with the stdout/stderr of the proc
    obj.enable_handlers = true
    if o.enable_handlers ~= nil then obj.enable_handlers = o.enable_handlers end
    ...
    obj.enable_recording = vim.F.if_nil(
        vim.F.if_nil(o.enable_recording, o.enable_handlers, o.enable_recording),
        true,
        o.enable_recording
    )

    if not obj.enable_handlers and obj.enable_recording then
        error('[test_harness.job] Cannot record items but disable handlers')
    end
...
function Job:_reset()
    ...
    if
        ...
        and vim.uv.is_active(self._shutdown_check)
        and not vim.uv.is_closing(self._shutdown_check)
    then
        vim.notify(
            debug.traceback(
                'We may be memory leaking here. This is now my problem.'
            ),
            vim.log.levels.ERROR
        )
    end
    self._shutdown_check = vim.uv.new_check()
...
function Job:_pipes_are_closed(options)
    for _, pipe in ipairs({ options.stdin, options.stdout, options.stderr }) do
        if pipe and not vim.uv.is_closing(pipe) then return false end
    end
...
function Job:shutdown(code, signal)
    if self._shutdown_check and vim.uv.is_active(self._shutdown_check) then
...
function Job:_shutdown(code, signal)
    if self.is_shutdown then return end

    self.code = code
    self.signal = signal

    if self._stdout_reader then pcall(self._stdout_reader, nil, nil, true) end

    if self._stderr_reader then pcall(self._stderr_reader, nil, nil, true) end

    if self._user_on_exit then self:_user_on_exit(code, signal) end

    for _, v in ipairs(self._additional_on_exit_callbacks) do
        v(self, code, signal)
    end

    if self.stdout then self.stdout:read_stop() end

    if self.stderr then self.stderr:read_stop() end
...
function Job:_create_uv_options()
    ...
    if self.detached then options.detached = true end
            ...
            if data then
                ...
                repeat
                    ...
                    if found_newline then
                        ...
                        if cb then cb(err, result_line, self) end
                        ...
                        then
                            ...
                            vim.schedule(function() self:shutdown() end)
...
function Job:_prepare_pipes()
    ...
    if writer then
        if Job.is_job(self.writer) and type(writer) ~= 'string' then
            writer:_prepare_pipes()
            self.stdin = writer.stdout
        elseif type(writer) ~= 'string' and writer.write then
            self.stdin = writer
        end
    end

    if not self.stdin then
        self.stdin = self.interactive and vim.uv.new_pipe(false) or nil
    end
...
function Job:_execute()
    ...
    if self._user_on_start then self:_user_on_start() end

    self.handle, self.pid =
        vim.uv.spawn(options.command, options, shutdown_factory(self, options))
    ...
    if writer then
        if Job.is_job(self.writer) and type(writer) ~= 'string' then
            writer:_execute()
        elseif
            type(self.writer) == 'table'
            and vim.islist(self.writer)
            and type(writer) ~= 'string'
        then
            local writer_len = #writer
            local stdin = self.stdin
            if stdin == nil then
                vim.notify(
                    'not sure why stdin would be null here',
                    vim.log.levels.WARN
                )
            elseif type(stdin) == 'string' then
                vim.notify(
                    'not sure why stdin would be a string here',
                    vim.log.levels.WARN
                )
            else
                for i, v in ipairs(writer) do
                    ...
                    if i ~= writer_len then
                        ...
                        stdin:write(
                            '\n',
                            function() pcall(self.stdin.close, self.stdin) end
                        )
                    end
                end
            end
        elseif type(writer) == 'string' then
            self.stdin:write(self.writer, function() self.stdin:close() end)
...
function Job:result()
    assert(
        ...
        "'enable_recording' is not enabled for this job."
    )
...
function Job:pid() return self.pid end
...
function Job:wait(timeout, wait_interval, should_redraw)
    ...
    local wait_result = vim.wait(timeout, function()
        if should_redraw then vim.cmd([[redraw!]]) end
...
function Job:co_wait(wait_time)
    ...
    if self.handle == nil then
        vim.notify(vim.inspect(self), vim.log.levels.ERROR)
        return
    end
...
function Job:and_then(next_job)
    self:add_on_exit_callback(function() next_job:start() end)
end

function Job:and_then_wrap(next_job)
    self:add_on_exit_callback(
        vim.schedule_wrap(function() next_job:start() end)
    )
end
...
function Job.chain(...)
    ...
    for index = 2, #jobs do
        ...
        prev_job:add_on_exit_callback(
            vim.schedule_wrap(function() job:start() end)
        )
    end

    local last_on_exit = jobs[#jobs]._user_on_exit
    jobs[#jobs]._user_on_exit = function(self, err, data)
        if last_on_exit then last_on_exit(self, err, data) end
...
function Job.chain_status(id) return _request_status[id] end
